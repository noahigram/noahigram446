...
class Fourier(Basis):
    ...
    def _transform_to_grid_real(self, data, axis, scale):
        ...
        if scale == 1:
            inv_four = scipy.fft.irfft(data)*self.N
            return inv_four[:self.N]
        
        else:
            self.scale=scale
            inv_real1 = scipy.fft.irfft(data,n=N_grid)*self.N*scale
            inv_real2 = scipy.fft.irfft(data)*self.N*scale
            print(len(inv_real2))

            return inv_real2[:N_grid]


    def _transform_to_coeff_real(self, data, axis):

        new_coeffs = scipy.fft.rfft(data,n=2*(self.N-1))
