...
class ImplicitTimestepper(Timestepper):
    ...
    def _LUsolve(self, data):
        if self.axis == 0:
            ...
            raise ValueError(
                "Can only do implicit timestepping on first or last axis")
...
class Multistage(ExplicitTimestepper):
    ...
    def __init__(self, eq_set, stages, a, b):
        ...
        for i in range(self.stages):
            self.X_list.append(StateVector(
                [np.copy(var) for var in self.X.variables]))
...
class StateVector:
    ...
    def scatter(self):
        for i, var in enumerate(self.variables):
            np.copyto(var, self.data[axslice(
                self.axis, i*self.N, (i+1)*self.N)])
